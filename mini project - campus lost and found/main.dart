      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text("Last Sign-In: ${user.metadata.lastSignInTime}"),
            const SizedBox(height: 24),

            // ---- Items Summary ----
            Text("My Item Statistics:",
                style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 8),

            StreamBuilder<QuerySnapshot>(
              stream: itemsRef.where('ownerId', isEqualTo: user.uid).snapshots(),
              builder: (context, snapshot) {
                if (!snapshot.hasData) {
                  return const Text("Loading item statistics...");
                }

                final docs = snapshot.data!.docs;
                final total = docs.length;
                final openItems = docs.where((d) => d['isClosed'] == false).length;
                final closedItems = total - openItems;

                return Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("Total Items Posted: $total"),
                    Text("Items Still Open: $openItems"),
                    Text("Returned / Claimed: $closedItems"),
                  ],
                );
              },
            ),
